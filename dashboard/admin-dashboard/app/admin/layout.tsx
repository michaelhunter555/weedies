...
import { fetchAdminChatUnreadCount } from "@/lib/admin-api";

const DRAWER_WIDTH = 260;

function MessagesNavItem({ selected }: { selected: boolean }) {
  const { accessToken, hydrated } = useAdminAuth();
  const { data } = useQuery({
    queryKey: ["admin-chats-unread"],
    queryFn: fetchAdminChatUnreadCount,
  ...
  return (
    <ListItemButton component={Link} href="/admin/messages" selected={selected}>
        ...
        <List dense sx={{ px: 1 }}>
          <ListItemButton
            ...
            selected={
              ...
              !pathname.startsWith("/admin/platform-listings")
            }
          >
            <ListItemIcon sx={{ minWidth: 40 }}>
          ...
          <ListItemButton
            component={Link}
            href="/admin/platform-listings/new"
            selected={pathname.startsWith("/admin/platform-listings")}
          >
            <ListItemIcon sx={{ minWidth: 40 }}>
              <AddCircleOutlineIcon fontSize="small" />
            </ListItemIcon>
            <ListItemText
              primary="Add platform listing"
              primaryTypographyProps={{ fontWeight: 500 }}
            />
          </ListItemButton>
          <MessagesNavItem selected={pathname.startsWith("/admin/messages")} />
          <ListItemButton
            component={Link}
            href="/admin/disputes"
